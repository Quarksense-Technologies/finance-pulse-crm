      ...
      <div className="px-4 py-4 sm:px-6 sm:py-6 space-y-6">
          ...
          <div className="mt-4 md:mt-0 flex flex-col sm:flex-row gap-4">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <button 
                  className="bg-primary text-white px-4 py-2 rounded-lg inline-flex items-center whitespace-nowrap"
        ...
        <div className="flex border-b border-gray-200 overflow-x-auto">
          {['all', 'in-progress', 'completed', 'on-hold'].map((filter) => (
            <button
              key={filter}
              className={`pb-2 px-4 whitespace-nowrap text-sm ${
                statusFilter === filter
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredProjects.length === 0 ? (
            ...
            filteredProjects.map((project) => (
              <div
                key={project.id}
                className="bg-white border border-gray-100 rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow relative"
              >
                {canDeleteProject && (
                ...
                <Link to={`/projects/${project.id}`} className="block">
                  ...
                  <div className="mt-4 border-t border-gray-100 pt-4 space-y-1">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-gray-900">{project.companyName || 'Unknown Company'}</span>
                    </div>
